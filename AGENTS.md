...
The compiled stylesheet is `web/resources/static/app.min.css` — built by
`npm run build` (Tailwind v4 CLI + `@plugin "daisyui"`) and regenerated by
the Dockerfile before `go build` embeds it. **Load `/static/app.min.css` in
your `.templ` `<head>`, NOT `daisyui.min.css`.** `daisyui.min.css` is a
DaisyUI **v4 standalone** relic that does not style DaisyUI **v5** markup
(`btn-primary`, themes via `data-theme`, utility classes) — loading it makes
the UI look unstyled/flat. `app.css` holds project-specific utilities (toast
container); load it after `app.min.css`.
...
- Do NOT emit a Datastar `PatchElements` / `patch-elements` SSE event whose
  fragment's top-level element has no `id` and no `WithSelector` — the JS
  client can't find a merge target and throws `PatchElementsNoTargetsFound`.
  Give the fragment's top-level element an `id` (e.g. `id="todo-list"`) AND
  pass `sdk.WithSelector("#todo-list")`. `internal/datastar.RenderAndPatch`
  is the shared helper; always pair it with a selector.
...
- **App session cookie is `gogogo_auth`, NOT PocketBase's `pb_auth`.**
  Reusing `pb_auth` makes app login clobber the PB superuser session (and
  vice-versa), and the admin UI throws "The authorized record is not
  allowed to perform this action." `LoadAuthFromCookie` also skips
  PocketBase's own surfaces (`/_/`, `/api/`, `/health`, `/static/`) so PB
  authenticates the admin/dashboard itself — if it set `e.Auth` from the
  app cookie on those paths, admin API calls would see a regular user
  record and be rejected. App page routes (`/`, `/todos`, `/login`,
  `/logout`) are NOT skipped, so they still get `e.Auth`.
...
> **Health check (scratch image).** The image is `scratch` — no shell, no
> `wget`/`curl`. The compose `healthcheck` uses exec form
> `CMD ["/app", "health"]` (never `CMD-SHELL`), where `/app health` is a
> built-in subcommand that GETs `/health` internally and exits 0/1. Don't
> switch it to `wget`/`curl` or `CMD-SHELL` — it marks the container
> permanently `(unhealthy)`. See PLAN.md Bug #12.
