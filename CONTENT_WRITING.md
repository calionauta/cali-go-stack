...
- **Pluggable UI skins as an orthogonal axis** — daisyui (default), basecoat (shadcn-style OKLCH tokens), and morpheus (vendorized web components) ship in the same binary. the active skin is selected at runtime via `UI_SKIN` env, per request via `?skin=` query, or interactively via the navbar `SkinSelector`. each skin is a `Skin{Name, Assets}` value registered at init via blank imports (`web/skins/skin.go`); the dispatcher falls back to daisyui when the env value is unknown. adding a fourth skin is: create `web/skins/<name>/`, register it from the import file, add a `make css-<name>` target. the contract is assets-only — templ templates stay in the feature.
- **SCOPE taxonomy as executable architecture** — every source file carries a `SCOPE:core` / `SCOPE:plugin` / `SCOPE:feature` annotation at the top. the annotation tells an agent not just what the file does but how to delete it. this is architecture as code, not as a wiki page that drifts. a feature file with `SCOPE:feature` is a file with a built-in delete instruction.
...
- **Routes as product surface, not plumbing** — `/` is the public marketing hero (no auth, no db read). `/todo` is the auth-gated todo demo. `/config` is the auth-gated read-only view of the running binary (masked secrets, runtime constants). `/_/` is the upstream pocketbase admin. `:8090` is the dagnats console. each route is a separate feature package (`features/landing/`, `features/config/`, etc.) with its own `RegisterRoutes(se)` on `se.Router` directly — the wiring is uniform across every page-style handler, so a future maintainer sees one consistent pattern. the routes are part of what the user sees; treating them as plumbing rather than surface area is how docs drift.

### III. Design Philosophy

- **Every decision documented, every decision replaceable** — `ARCHITECTURE.md` reads like a team's decision log, not a system diagram. each component has a "remove by" instruction. the design rationale in `ARCHITECTURE.md` collects the war stories behind the weirder choices (the two cookies, the pocketbase realtime vs sse hub split, the build-tag absence). the template is a collection of agreements someone already had, written down.
...
- **Documentation for agents, not just humans** — scope annotations, the `ARCHITECTURE.md` entrypoint, the lint config tuned to llm mistakes — these exist to be read by ai agents navigating the codebase. the template treats agents as first-class readers of its documentation, with explicit paths for them to read first.
...
- **annotated decision log excerpt** — pairs with: senior engineers. quotes `ARCHITECTURE.md` and unpacks one decision at a time.

### IX. What This Template Is Not (anti-marketing)

this block exists because half the value of the prose is the negative space.

- **not a framework.** a framework locks you in. this is a collection of replaceable choices. you read `ARCHITECTURE.md` and decide whether to keep each.
...
this is a collection of decisions you can read, argue with, and replace. the binary ships what the choices ship. when you outgrow a choice, the `ARCHITECTURE.md` tells you where to start. that is the whole offer.
